...
export class Rendertron {
  ...
  async initialize() {
    ...
    if (this.config.datastoreCache) {
      if (this.config.cacheTool === 'redis') {
        const { promisify } = await import('util');
        const redis = await import('redis'),
          redisClient = redis.createClient({
            host: (process.env.REDIS_HOST as string) || this.config.redisHost,
            port:
        ...
        this.app.use(
          async (ctx: Koa.Context, next: () => Promise<undefined>) => {
            const url = ctx.url;
            ...
            if (ctx.status === 200) {
              redisClient.set(url, ctx.body, 'ex', this.config.redisExpireTime);
            }
          }
        );
      }
